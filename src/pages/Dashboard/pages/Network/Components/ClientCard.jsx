...
function ClientCard({ name, maxBandwidth, bandwidthLimit, isOnline }) {
  return (
    <div className="relative flex flex-col w-3/12 max-md:ml-0 max-md:w-full">
      <div className="absolute top-4 right-4 w-4 h-4 rounded-full border-2 border-white" style={{ backgroundColor: isOnline ? 'green' : 'orange' }}></div>
      <div className="flex flex-col grow text-xl text-stone-700">
        <div className="flex flex-col pt-3.5 pb-10 rounded-3xl bg-slate-300">
          <div className="self-center text-2xl font-medium text-black">{name}</div>
